...
export class GetAverageDeliveryTimeForCompletedOrderUseCase {
    ...
    public async call(restaurant_id: string, period: string) {
        ...
        let response;
        const ordersList = await this.orderService.getAllCompletedOrdersByRestaurantId(restaurant_id);
    ...
    private groupByPeriod(deliveryTimes: Array<{ createdAt: Date, deliveryTime: number }>, period: string) {
        return deliveryTimes.reduce((acc, order) => {
