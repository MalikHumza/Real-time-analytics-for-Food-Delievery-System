...
export class DeliverOrderUseCase {
    ...
    public async call(req: RequestWithUser, order_id: string) {
        ...
        const get_delivery = await this.deliveryService.getDeliveryByOrder(user_id, order_id);
        const update_delivery = await this.deliveryService.updateDeliveryTime(user_id, get_delivery.id, order_id, current_time);
        await this.orderService.updateOrderStatus(order.user_id, update_delivery.order_id, order.restaurant_id, ORDER_STATUS.COMPLETED);
