...
export class GetPeakOrderTimeForEachRestaurantUseCase {
    ...
    public async call() {
        ...
        for (let index = 0; index < restaurants.length; index++) {
            ...
            const ordersList = await this.orderService.getAllOrdersByRestaurantId(item.id);
            if (ordersList.length === 0) {
                continue;
            }

            // group orders by hour
            const hourlyOrderCount = ordersList.reduce((acc, order) => {
            ...
            const peakTime = Object.entries(hourlyOrderCount).reduce((max, [hour, count]) => {
