...
export class CreateOrdersUseCase {
    ...
    public async call(req: RequestWithUser, restaurant_id: string, data: CreateOrdersDTO) {
        ...
        const create_order = await this.orderService.createOrder({ order_time: current_time, user_id, restaurant_id, status: ORDER_STATUS.PENDING, dishes: data.dishes.map(dish => ({ dish_id: dish.dish_id, quantity: dish.quantity })) })
